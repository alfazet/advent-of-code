...
pub fn solve(input: &str) -> u32 {
    ...
    let mut sources: Vec<(usize, usize, Dir)> = Vec::new();
    for r in 0..n {
        sources.push((r, 0, Dir::East));
        sources.push((r, m - 1, Dir::West));
    }
    for c in 0..m {
        sources.push((0, c, Dir::South));
        sources.push((n - 1, c, Dir::North));
    }

    let ans = sources.par_iter().map(|(r, c, dir)| {
        let mut q: Vec<State> = Vec::new();
        q.push(State { r: *r, c: *c, dir: *dir });
